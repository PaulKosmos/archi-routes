          ...
          <div className="flex gap-2 mb-6 border-b border-gray-200">
            {(['all', 'published', 'draft'] as const).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2 font-medium border-b-2 transition-colors ${filter === f
                ...
                <tbody className="divide-y divide-gray-200">
                  {episodes.map(episode => (
                    <tr key={episode.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${episode.status === 'published'
