...
export default function AddObjectPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
   ...
   try {
     ...
     // Закомментируем загрузку фото для тестирования
     /*
     if (buildingPhotoFiles.length > 0) {
       console.log('📸 Начинаем загрузку фотографий здания...')
       
       for (const [index, file] of buildingPhotoFiles.entries()) {
         console.log(`📸 Обрабатываем фото ${index + 1}/${buildingPhotoFiles.length}`)
         console.log(`   - Имя файла: ${file.name}`)
         console.log(`   - Размер: ${(file.size / 1024 / 1024).toFixed(2)} MB`)
         console.log(`   - Тип: ${file.type}`)
         
         const fileName = `buildings/${Date.now()}-${index}-${file.name.replace(/[^a-zA-Z0-9.-]/g, '_')}`
         console.log(`   - Путь загрузки: ${fileName}`)
         ...
         try {
           ...
           if (photoError) {
             console.error(`   ❌ Ошибка загрузки фото ${index + 1}:`, photoError)
             console.error(`   📋 Детали ошибки:`, {
               message: photoError.message,
               statusCode: photoError.statusCode,
               error: photoError.error
             })
             // Продолжаем выполнение даже если фото не загрузилось
           } else {
             console.log(`   ✅ Фото ${index + 1} загружено успешно:`, photoData)
             ...
             buildingImageUrls.push(publicUrl)
             console.log(`   🔗 Публичный URL: ${publicUrl}`)
           }
         } catch (uploadError) {
           console.error(`   💥 Исключение при загрузке фото ${index + 1}:`, uploadError)
         }
       }
     } else {
       console.log('📸 Фотографии здания отсутствуют, пропускаем')
     }
     */
