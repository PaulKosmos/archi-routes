...
export default function BuildingReviewsList({ 
  ...
  const handleRateReview = async (reviewId: string, rating: number) => {
    if (!user) {
      toast.error('Войдите, чтобы оценить обзор')
      return
    }
  ...
  if (reviews.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 mb-4">📭 Пока нет обзоров</p>
        {onOpenAddReview && (
          <button
      ...
      {reviews.map(review => {
        ...
        return (
          ...
          >
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white font-semibold">
                  {review.profiles?.avatar_url ? (
                    <img
                      ...
                      alt={review.profiles.full_name || 'Аватар'}
                ...
                <div>
                  <p className="font-semibold text-gray-900">
                    {review.profiles?.full_name || review.profiles?.username || 'Пользователь'}
                  </p>
            ...
            {ratingCount > 0 && (
              <div className="flex items-center mb-3 bg-yellow-50 px-3 py-2 rounded-lg">
                ...
                <span className="text-sm text-gray-600 ml-2">({ratingCount} оценок)</span>
              </div>
            )}
                ...
                {review.content.length > 300 && (
                  ...
                  >
                    {isExpanded ? (
                      ...
                      <>Показать полностью <ChevronDown className="w-4 h-4 ml-1" /></>
                    )}
                  </button>
                )}
              </div>
            ...
            <div className="border-t border-gray-100 pt-4 mt-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                {/* Оценка звездами */}
                <div>
                  <p className="text-xs text-gray-600 mb-2">Оцените обзор:</p>
                ...
                >
                  <ThumbsUp className={`w-4 h-4 mr-1 ${helpfulVotes.has(review.id) ? 'fill-current' : ''}`} />
                  <span className="text-sm">
                    {helpfulVotes.has(review.id) ? 'Полезно' : 'Полезно?'} ({review.helpful_count})
                  </span>
                </button>
              </div>
            </div>
